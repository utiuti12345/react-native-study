...
function EditableRow(props:EditableProps) {
    const {state,isEditable,actions:{toggleTodo,removeTodo,gotoDetail}} = props;
    ...
    return(
        <SwipeRow disableLeftSwipe={!isEditable}
...
function ReadonlyRow(props:ReadonlyProps) {
    const {state, isEditable, actions: {gotoDetail}} = props;
    const onPress = React.useCallback(() => {
        gotoDetail(state, isEditable)
    }, [state, isEditable, gotoDetail]);
